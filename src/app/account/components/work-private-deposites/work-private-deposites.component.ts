...
@Component({
  selector: 'app-work-private-deposites',
  templateUrl: './work-private-deposites.component.html',
  styleUrls: ['./work-private-deposites.component.scss']
})
export class WorkPrivateDepositesComponent implements OnInit {
  ...
  loadData() {
    this.loading = true;
    this.globalService.get('account/transformations_private', this.filter).subscribe((res) =>{
      this.data = res;
      this.calculateTotal();
      this.create('');
      this.loading = false;
