...
@Component({
  selector: 'app-customcert',
  templateUrl: './customcert.component.html',
  styleUrls: ['./customcert.component.scss']
})
export class CustomcertComponent implements OnInit {

  $: any = $;
  doc: any = document;
  isSubmitted = false;
    isSubmittedPrint = false;
   ...
   load() {
    ...
    if ( this.student.id) {
      ...
      this.globalService.loadHtmlwithoutToken(
        ...
        $('#reportContent').html(res);
        this.isSubmittedPrintafter=true
      });
    } else {
